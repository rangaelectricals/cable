    ...
    container.innerHTML = `
      <div class="space-y-6 page-enter">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ${this._groups.map(g => `
          <div class="card bg-white shadow-sm border border-slate-200 hover:border-primary transition-colors cursor-pointer group"
            ...
            <div class="card-body p-4">
              <div class="flex justify-between items-start mb-3">
                ...
                <div class="text-right">
                  ...
                  <div class="text-sm text-slate-500">${Helpers.escape(g.sqmm)} mm²</div>
                </div>
              </div>

              <div class="grid grid-cols-2 gap-4 border-t border-base-100 pt-3">
                <div>
                  <div class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Quantity</div>
                  <div class="text-sm font-bold">${g.count} <span class="text-[10px] font-normal opacity-50">Cables</span></div>
                </div>
                <div>
                  <div class="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Length</div>
                  <div class="text-sm font-bold text-indigo-600">${Math.round(g.totalMeter)}m</div>
                </div>
              </div>
