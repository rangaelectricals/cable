...
const LogsPage = {
  ...
  _renderGrid() {
    ...
    const meta = {
      ACTIVATE:         { icon:'check-circle', bg:'bg-primary/8',  text:'text-primary', badge:'badge-primary', label:'Activate' },
      SEND_TO_SITE:     { icon:'truck',         bg:'bg-warning/8',  text:'text-warning', badge:'badge-warning', label:'Send'     },
      RETURN_TO_GODOWN: { icon:'warehouse',     bg:'bg-success/8',  text:'text-success', badge:'badge-success', label:'Return'   },
    };

    wrap.innerHTML = this._logs.map(l => {
      const m = meta[l.action] || { icon:'circle', bg:'bg-base-200', text:'text-base-content/30', badge:'badge-ghost', label:l.action };
      return `
      <div class="bg-white border border-base-200 rounded-xl overflow-hidden flex flex-col shadow-sm">

        <!-- Coloured header strip -->
        <div class="${m.bg} border-b border-base-200 px-2.5 py-2 flex items-center gap-2">
        ...
        <div class="px-2.5 py-2.5 flex-1 space-y-1">
          ...
          <div class="flex items-center gap-1 text-[10px] text-base-content/45">
          ...
          <div class="text-[10px] text-base-content/30">${Helpers.timeAgo(l.timestamp)}</div>
        </div>

        <!-- Timestamp footer -->
        <div class="border-t border-base-100 px-2.5 py-1.5">
          <span class="text-[10px] font-mono text-base-content/25">${Helpers.formatDateTime(l.timestamp)}</span>
        </div>
      </div>`;
    }).join('');
  ...
  _renderTable() {
    ...
    <tr class="hover">
      ...
      <td class="text-sm text-base-content/60 max-w-[160px] truncate">
...
const UsersPage = {
  ...
  async render(container) {
    ...
    <div class="space-y-4 page-enter">
      ...
      <!-- DESKTOP: table (hidden on mobile) -->
      <div class="hidden sm:block card bg-base-100 shadow-sm border border-base-200 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="table table-sm table-zebra">
            <thead class="bg-base-200/60 text-xs uppercase tracking-wide">
              <tr>
          ...
          <div class="space-y-3">
            ${UI.field('Full Name',
              `<label class="input input-bordered flex items-center gap-2">
                <i data-lucide="user" class="w-4 h-4 text-base-content/40 shrink-0"></i>
                <input type="text" id="u-name" class="grow" placeholder="John Doe" required />
              </label>`, true)}
            ${UI.field('Username',
              `<label class="input input-bordered flex items-center gap-2">
                <i data-lucide="at-sign" class="w-4 h-4 text-base-content/40 shrink-0"></i>
                <input type="text" id="u-username" class="grow" placeholder="john_doe" required autocomplete="off" />
              </label>`, true)}
            ${UI.field('Password',
              `<label class="input input-bordered flex items-center gap-2">
                <i data-lucide="lock" class="w-4 h-4 text-base-content/40 shrink-0"></i>
                <input type="password" id="u-password" class="grow" placeholder="Password" autocomplete="new-password" />
              </label>
              <label class="label pt-1">
                <span id="u-pwd-hint" class="label-text-alt text-base-content/40">Required for new user</span>
              </label>`)}
            ${UI.field('Role',
  ...
  _renderGrid() {
    ...
    if (!paged.length) {
      wrap.innerHTML = `<div class="col-span-2 bg-white border border-base-200 rounded-xl">
    ...
    wrap.innerHTML = paged.map(u => {
      ...
      <div class="bg-white border border-base-200 rounded-xl overflow-hidden flex flex-col shadow-sm">
        ...
        <!-- Avatar header -->
        <div class="bg-primary/5 border-b border-base-200 px-2.5 py-3 flex flex-col items-center gap-1.5">
        ...
        <div class="px-2.5 py-2 flex-1 min-w-0 space-y-0.5">
          ...
          <div class="text-[10px] font-mono text-base-content/40 truncate">@${Helpers.escape(u.username)}</div>
  ...
  _renderTable() {
    ...
    body.innerHTML = paged.map((u,i) => {
      ...
      <tr class="hover group">
        <td class="text-base-content/35 text-xs">${start+i+1}</td>
        <td>
