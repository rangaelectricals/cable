...
const ScanPage = {
  ...
  async render(container) {
    ...
    let sites = [], officers = [];
    try {
      const res = await API.getProducts({ pageSize: 2000 });
      if (res.data) {
        sites = [...new Set(res.data.map(p => p.siteName).filter(Boolean))];
        officers = [...new Set(res.data.map(p => p.personAssigned).filter(Boolean))];
      }
    } catch (e) {}
            ...
            <div class="bg-white rounded-xl shadow border border-slate-200/60 overflow-visible relative">
              ...
              <div class="p-4 sm:p-6 space-y-4">
                  ...
                  <div class="bg-slate-50 border border-slate-200/60 rounded-xl p-4 space-y-4">
                    <div id="extra-site" class="hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div class="space-y-1">
                        ...
                        <input type="text" id="f-site-name" list="datalist-sites" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm font-semibold text-slate-800 focus:border-slate-900 outline-none transition-all" placeholder="Search site..." />
                      </div>
                      <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Assignment Officer</label>
                        <input type="text" id="f-person" list="datalist-officers" class="w-full bg-white border border-slate-200 rounded-lg px-4 py-2 text-sm font-semibold text-slate-800 focus:border-slate-900 outline-none transition-all" placeholder="Name..." />
                      </div>
                      <div class="space-y-1">
                ...
                <div class="space-y-4">
                  ...
                  <div id="wrap-continuous-scan" class="flex items-center justify-between bg-slate-50 border border-slate-200/60 rounded-xl px-4 py-3 mt-1 animate-slideDown">
                    <div class="flex items-center gap-3">
                      <div class="w-7 h-7 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center">
                        <i data-lucide="refresh-cw" class="w-4 h-4"></i>
                      </div>
                      <div>
                        <p class="text-xs font-bold text-slate-700 uppercase tracking-tight">Continuous Scanning</p>
                        <p class="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Automate sequential back-to-back operations</p>
                      </div>
                    </div>
                    <input type="checkbox" id="continuous-scan-toggle" class="toggle toggle-primary toggle-sm" />
                  </div>
    ...
    <datalist id="datalist-sites">
      ${sites.map(s => `<option value="${Helpers.escape(s)}">`).join('')}
    </datalist>
    <datalist id="datalist-officers">
      ${officers.map(o => `<option value="${Helpers.escape(o)}">`).join('')}
    </datalist>
  ...
  async trigger() {
    ...
    const continuous = document.getElementById('continuous-scan-toggle')?.checked;
    if (continuous && successCount > 0) {
      try {
        const actx = new (window.AudioContext || window.webkitAudioContext)();
        const osc = actx.createOscillator();
        const gain = actx.createGain();
        osc.type = 'sine'; osc.frequency.setValueAtTime(660, actx.currentTime);
        gain.gain.setValueAtTime(0.1, actx.currentTime);
        osc.connect(gain); gain.connect(actx.destination);
        osc.start(); osc.stop(actx.currentTime + 0.1);
      } catch (e) {}

      Toast.show('success', 'Processed', `${successCount} items processed successfully.`);
